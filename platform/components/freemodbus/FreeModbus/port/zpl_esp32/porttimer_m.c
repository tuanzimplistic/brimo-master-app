...
#if defined(CONFIG_MODBUS_ZPL_IDF_V4_0)
#define MAKE_TIMER_GROUP(x) TIMERG##x
#define TIMER_GROUP_DEV(x)  MAKE_TIMER_GROUP(x)
#define TIMER_GROUP_DEVICE  TIMER_GROUP_DEV(CONFIG_FMB_TIMER_GROUP)
#endif /* #if defined(CONFIG_MODBUS_ZPL_IDF_V4_0) */
...
static void IRAM_ATTR vTimerGroupIsr(void *param)
{
    ...
    uint32_t intr_status = TIMER_GROUP_DEVICE.int_st_timers.val;
    if(intr_status & BIT(usTimerIndex)) {
        if(usTimerIndex == TIMER_0) {
            TIMER_GROUP_DEVICE.int_clr_timers.t0 = 1;
        } else if(usTimerIndex == TIMER_1) {
            TIMER_GROUP_DEVICE.int_clr_timers.t1 = 1;
        }
    }
#else
#error "Invalid IDF for modbus master"
#endif
...
#elif defined(CONFIG_MODBUS_ZPL_IDF_V4_0)
    TIMER_GROUP_DEVICE.hw_timer[usTimerIndex].config.alarm_en = TIMER_ALARM_EN;
#endif
}
...
#elif defined(CONFIG_MODBUS_ZPL_IDF_V4_0)
        TIMER_GROUP_DEVICE.hw_timer[usTimerIndex].config.enable = TIMER_PAUSE;
#endif
    } else {
