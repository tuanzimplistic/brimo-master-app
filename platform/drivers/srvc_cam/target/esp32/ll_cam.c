...
static void IRAM_ATTR ll_cam_vsync_isr (GPIO_evt_data_t * pstru_evt_data)
{
    //DBG_PIN_SET(1);
    cam_obj_t *cam = g_cam;
    ...
    ets_delay_us(1);
    uint8_t u8_level;
    s8_GPIO_Read_Level (pstru_evt_data->x_inst, &u8_level);
    if (u8_level == !cam->vsync_invert) {
...
{
    GPIO_inst_t x_vsync;
    s8_GPIO_Get_Inst (GPIO_CSI_VSYNC, &x_vsync);

    if (en) {
        printf("Enable Vsync interrupt\n");
        s8_GPIO_Enable_Interrupt (x_vsync, GPIO_INT_FALLING_EDGE, ll_cam_vsync_isr, NULL);
        g_cam = cam;
    } else {
        printf("Disable Vsync interrupt\n");
        s8_GPIO_Disable_Interrupt (x_vsync);
    }
}
